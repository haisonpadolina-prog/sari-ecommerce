...
@section('header-subtitle', 'Live Delivery Operations')
...
<div class="mx-auto max-w-[1620px]">
    ...
    <section class="mb-4 flex flex-col justify-between gap-4 border-b border-[#eadfc9] pb-5 sm:flex-row sm:items-end">
        <div>
            <div class="flex items-center gap-2"><h2 class="text-[15px] font-bold text-[#211d17]">Courier Operations</h2><span class="rounded-full border border-[#eadfc9] bg-white px-2.5 py-1 text-[8px] font-semibold text-[#8a7f70]">MySQL Live Flow</span></div>
            <p class="mt-2 text-[9px] text-[#817769]">Accept Seller pickup requests, confirm pickup, deliver the package, and update the Seller automatically.</p>
        </div>
        <a href="{{ route('courier.requests') }}" class="rounded-xl bg-[#D99600] px-4 py-2.5 text-[9px] font-semibold text-white">View Delivery Requests →</a>
    </section>

    <section class="grid grid-cols-2 gap-3 md:grid-cols-4">
        @foreach([
            ['Available Requests', $stats['available_requests'], 'bg-[#fff2d8] text-[#bb7b00]'],
    ...
    <section class="mt-4 overflow-hidden rounded-[20px] border border-[#eadfc9] bg-[#fffdf8]">
        <div class="border-b border-[#eadfc9] px-5 py-4"><h3 class="text-[13px] font-bold text-[#211d17]">Current Delivery</h3><p class="mt-1 text-[8px] text-[#918677]">The tracker moves only after the courier confirms each step.</p></div>
        ...
        @if ($currentDelivery instanceof \App\Models\MarketplaceOrder)
            ...
            <div class="p-5">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <div class="flex flex-wrap items-center gap-2"><h4 class="text-[13px] font-bold text-[#201c16]">{{ $currentDelivery->order_number }}</h4><span class="rounded-full border border-[#d2dfe9] bg-[#f2f7fb] px-2.5 py-1 text-[7px] font-bold text-[#3C6E91]">{{ strtoupper($currentDelivery->statusLabel()) }}</span></div>
                        <p class="mt-2 text-[8px] text-[#817769]">{{ $currentDelivery->pickup_name }} → {{ $currentDelivery->buyer_name }}</p>
                    </div>
                    <div class="rounded-xl border border-[#eadfc9] bg-white px-4 py-2.5"><p class="text-[7px] text-[#918677]">Courier Fee</p><p class="mt-1 text-[12px] font-bold text-[#201c16]">₱{{ number_format((float)$currentDelivery->delivery_fee,2) }}</p></div>
                </div>
                ...
                <div class="mt-6 grid grid-cols-3 gap-3">
                    <div class="rounded-[16px] border {{ $pickupDone ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50' }} p-4 text-center">
                        ...
                        <p class="mt-3 text-[9px] font-bold text-[#2d2822]">Picked Up</p><p class="mt-1 text-[7px] text-[#8f8578]">Seller handoff</p>
                    </div>
                    <div class="rounded-[16px] border {{ $transitDone ? 'border-emerald-200 bg-emerald-50' : ($status === 'in_transit' ? 'border-blue-200 bg-blue-50' : 'border-[#e5ddd1] bg-white') }} p-4 text-center">
                        ...
                        <p class="mt-3 text-[9px] font-bold text-[#2d2822]">In Transit</p><p class="mt-1 text-[7px] text-[#8f8578]">On the way</p>
                    </div>
                    <div class="rounded-[16px] border {{ $deliveredDone ? 'border-emerald-200 bg-emerald-50' : 'border-[#e5ddd1] bg-white' }} p-4 text-center">
                        ...
                        <p class="mt-3 text-[9px] font-bold text-[#2d2822]">Delivered</p><p class="mt-1 text-[7px] text-[#8f8578]">Buyer handoff</p>
                    </div>
                </div>

                <div class="mt-5 rounded-[16px] border border-[#eadfc9] bg-white p-4">
                    @if ($status === 'courier_accepted')
                        <p class="text-[8px] font-semibold text-[#8a6400]">Request accepted. Start traveling to the Seller.</p>
        ...
        @else
            <div class="p-10 text-center">
                @if (is_array($currentDelivery))
                    <div class="mx-auto mb-4 max-w-[560px] rounded-xl border border-[#ead8d2] bg-[#fff7f3] px-4 py-3 text-[8px] leading-5 text-[#9a6156]">
                        Legacy array-based courier data was detected. The database-backed
                        <strong>CourierDashboardController</strong> included in this fix
                        must replace the old controller.
                    </div>
                @endif

                <p class="text-[10px] font-semibold text-[#51483d]">No active delivery.</p>
                <p class="mt-2 text-[8px] text-[#918677]">Accept a Seller request to start the pickup workflow.</p>
                <a href="{{ route('courier.requests') }}" class="mt-4 inline-flex rounded-xl bg-[#D99600] px-5 py-3 text-[9px] font-semibold text-white">Open Requests</a>
            </div>
        @endif
    </section>

    <section class="mt-4 rounded-[20px] border border-[#eadfc9] bg-[#fffdf8] p-5">
        <div class="flex items-center justify-between"><div><h3 class="text-[12px] font-bold text-[#211d17]">Available Seller Requests</h3><p class="mt-1 text-[8px] text-[#918677]">Latest Ready for Pickup orders.</p></div><a href="{{ route('courier.requests') }}" class="text-[8px] font-semibold text-[#a66e00]">View all →</a></div>
        <div class="mt-4 space-y-2">
            @forelse($requests as $requestOrder)
                <div class="flex items-center justify-between gap-3 rounded-xl border border-[#eadfc9] bg-white p-3">
                    <div><p class="text-[9px] font-bold text-[#2a251f]">{{ $requestOrder->order_number }}</p><p class="mt-1 text-[7px] text-[#84796c]">{{ $requestOrder->pickup_name }} → {{ $requestOrder->buyer_name }}</p></div>
                    <p class="text-[9px] font-bold text-[#2b261f]">₱{{ number_format((float)$requestOrder->delivery_fee,2) }}</p>
                </div>
            @empty
                <div class="rounded-xl border border-dashed border-[#ddd2c1] p-5 text-center text-[8px] text-[#918677]">No waiting requests.</div>
            @endforelse
        </div>
    </section>
</div>
